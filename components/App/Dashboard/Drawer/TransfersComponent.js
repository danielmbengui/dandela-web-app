...
import { PAGE_LINK_INPROGRESS_LIST, PAGE_LINK_NEW_TRANSFER, USER_TYPE_ADMIN, USER_TYPE_EMPLOYE_ANGOLA } from '../../../../constants';
import { useTranslation } from 'react-i18next';

export default function TransfersComponent(props) {
    const { user, openSub, pages, } = props;
    ...
    function createMenu() {
        ...
        if (user) {
            if (user.type !== USER_TYPE_EMPLOYE_ANGOLA) {
                menu.push({ icon: <AddCircleIcon fontSize='large' />, label: t('menuTransfertsNew'), link: PAGE_LINK_NEW_TRANSFER, active: pages.addtransfertPage });
            }
            menu.push({ icon: <CurrencyExchangeIcon fontSize='large' />, label: t('menuTransfertsInProgress'), link: PAGE_LINK_INPROGRESS_LIST, active: pages.inprogressPage });
